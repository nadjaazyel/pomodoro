  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#202126] text-zinc-300">
      ...
      <div className="px-5 py-3.5 border-b border-[#353745] bg-[#282a34] flex items-center justify-between shrink-0">
        ...
        <button
          onClick={onOpenFiles}
          className="flex items-center space-x-2 px-3.5 py-2 rounded-xl bg-[#333644] border border-[#414456] text-zinc-200 hover:text-white hover:border-zinc-400 text-xs sm:text-sm font-semibold transition-all active:scale-95 shadow-sm"
      ...
      <div className="px-4 py-3 border-b border-[#303340] shrink-0">
        <div className="flex items-center space-x-2.5 bg-[#282a34] border border-[#383b4a] rounded-2xl px-3.5 py-2 focus-within:border-zinc-400 transition-colors">
      ...
      {/* Lista de Contatos */}
      <div className="flex-1 overflow-y-auto px-3 py-2 divide-y divide-[#2c2e3a]">
        {filteredContacts.map((contact) => (
          <button
            key={contact.id}
            onClick={() => onSelectContact(contact)}
            className="w-full p-3.5 sm:p-4 rounded-2xl hover:bg-[#282a34] transition-all flex items-center justify-between text-left group active:scale-[0.99]"
          >
            <div className="flex items-center space-x-3.5 min-w-0">
              {/* Avatar Neutro */}
              <div className="relative shrink-0">
                <div className="w-12 h-12 rounded-2xl bg-[#313442] border border-[#414456] flex items-center justify-center font-mono text-sm font-bold text-zinc-200 shadow-sm">
                  {contact.avatar}
                </div>
                {contact.online && (

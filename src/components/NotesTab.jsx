...
export default function NotesTab() {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-black text-gray-300">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-900/80 bg-black/70 backdrop-blur-sm z-10">
        <div>
          <h2 className="text-xs font-mono uppercase tracking-wider text-gray-200">
            Acordos Registrados ({notes.length})
          </h2>
          <p className="text-[10px] text-zinc-500">Documentos salvos e notas cifradas</p>
        </div>

        {/* Botão Novo Acordo no Topo */}
        <button
          onClick={() => setIsModalOpen(true)}
          className="flex items-center space-x-1.5 px-3 py-1.5 rounded-full bg-zinc-900 border border-gray-800 hover:border-zinc-700 text-zinc-300 hover:text-white text-xs transition-all active:scale-95"
        >
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
        {notes.map((note) => (
          <div
            key={note.id}
            className="rounded-xl border border-gray-900 bg-zinc-950/80 p-4 transition-all hover:border-zinc-800"
          >
            <div className="flex items-start justify-between gap-2 mb-2">
              ...
              <span className="text-[9px] font-mono px-2 py-0.5 rounded-full bg-zinc-900 border border-gray-800 text-zinc-400 shrink-0">
                {note.status}
              </span>
            </div>
            ...
            <div className="mt-3 pt-2.5 border-t border-gray-900/60 flex items-center justify-between text-[10px] font-mono text-zinc-600">

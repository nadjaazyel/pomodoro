...
export default function ChatTab() {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-black text-gray-300">
      {/* Sub-header confidencial com o interlocutor K9 */}
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-900/80 bg-black/70 backdrop-blur-sm z-10">
        <div className="flex items-center space-x-3">
          <div className="relative">
            <div className="w-8 h-8 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center text-xs font-mono font-semibold text-zinc-300">
              K9
            </div>
            <div className="absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full bg-emerald-500 ring-2 ring-black" />
          </div>
          <div className="flex flex-col">
            <span className="text-xs font-medium text-gray-200 tracking-wide">K9</span>
            <span className="text-[10px] text-zinc-500 font-mono">E2EE ativo • Cifrado</span>
          </div>
        </div>

        <div className="text-[10px] text-zinc-600 font-mono flex items-center space-x-1">
          ...
          >
            <div
              className={`max-w-[82%] sm:max-w-md rounded-2xl px-3.5 py-2.5 text-xs sm:text-sm leading-relaxed ${
                msg.isMe
                  ? 'bg-zinc-850 text-gray-200 rounded-br-xs border border-zinc-800/80'
                  : 'bg-zinc-950 text-gray-300 rounded-bl-xs border border-gray-900'
              }`}
            >
              {!msg.isMe && (
                <div className="text-[10px] font-mono text-zinc-500 mb-1 font-semibold">
                  {msg.sender}
                </div>
              )}
              ...
              <div
                className={`text-[9px] font-mono mt-1 text-right ${
                  msg.isMe ? 'text-zinc-400' : 'text-zinc-600'
                }`}
              >
                {msg.time}
              </div>
      ...
      */}
      <div className="border-t border-gray-900 bg-black/90 p-2.5">
        <form
          onSubmit={handleSendMessage}
          className="flex items-center space-x-2 w-full max-w-2xl mx-auto"
        >
          <div className="relative flex-1 flex items-center bg-zinc-950 border border-gray-900 rounded-full px-3.5 py-1.5 focus-within:border-zinc-700 transition-colors">
            <input
              type="text"
              value={inputText}
              onChange={(e) => setInputText(e.target.value)}
              placeholder="Mensagem..."
              className="w-full bg-transparent text-xs sm:text-sm text-gray-200 placeholder-zinc-600 outline-none pr-7 py-1"
            />

            {/* Ícone de Microfone minúsculo e quase invisível */}
            <button
              type="button"
              className="absolute right-3 text-zinc-500 opacity-30 hover:opacity-70 transition-opacity p-0.5"
          ...
          <button
            ...
            className={`p-2 rounded-full transition-all flex items-center justify-center ${
              inputText.trim()
                ? 'bg-zinc-200 text-black active:scale-95'
